...
export default function ArticleCard({ articulo }) {
  // Buscamos el título principal del artículo.
  // Si no existe title_es, usamos title. Si tampoco existe, mostramos un texto por defecto.
  const titulo =
    articulo.title_es ||
    articulo.title ||
    "Artículo sin título";
  ...
  const cambiarFavorito = () => {
    ...
    if (favoritosActuales.includes(articulo.id)) {
      nuevosFavoritos = favoritosActuales.filter(
        (idFavorito) => idFavorito !== articulo.id
      );
      setEsFavorito(false);
    } else {
